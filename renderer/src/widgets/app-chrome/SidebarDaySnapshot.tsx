...
export function SidebarDaySnapshot({ compact = false }: { compact?: boolean } = {}) {
  ...
  const snap = useMemo(() => {
    ...
    const kcal = daySnapshot
      ...
      : (bootstrapData?.nutritionEntries ?? db.getNutritionEntries(dateString)).reduce((acc, e) => acc + Math.max(0, Number(e.total_calories) || 0), 0);
    ...
    const done = sunrise.completed + sunset.completed;
    const total = sunrise.total + sunset.total;
    ...
    const balanceLabel = formatAmount(financeSnapshot.balance, currency);
    ...
    <section className="mb-2 shrink-0 overflow-hidden rounded-lg border border-[var(--aura-border-soft)] bg-transparent" aria-label="Сводка по выбранному дню">
      <div className={cn('grid grid-cols-[minmax(0,1fr)_auto] items-end gap-3 px-3', isDense ? 'py-2' : 'py-2.5')}>
        ...
        {snap.progressPct != null ? (
          <div className="text-right leading-none">
            <p className={cn('font-mono font-semibold tabular-nums text-primary', isDense ? 'text-sm' : 'text-[15px]')}>{snap.progressPct}%</p>
      ...
      {snap.progressPct != null ? (
        <div className="border-t border-[var(--aura-border-soft)] px-3 py-2">
          <Progress value={snap.progressPct} className="h-1 bg-border/55 [&_[data-slot=progress-indicator]]:bg-primary" />
        </div>
      ) : (
