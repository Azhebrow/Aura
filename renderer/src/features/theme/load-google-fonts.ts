import { AURA_FONT_CHOICES, AURA_FONT_STANDARD, AURA_GOOGLE_FONT_WEIGHTS } from '@/features/theme/font-constants';
...
function createGoogleFontsHref(): string {
  const params = AURA_FONT_CHOICES.filter((font) => font !== AURA_FONT_STANDARD).map(
    (font) => `family=${font.replaceAll(' ', '+')}:wght@${AURA_GOOGLE_FONT_WEIGHTS[font] ?? '400;600;700'}`
  );
