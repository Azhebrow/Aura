/** Единое событие для инвалидации кэша статистики (renderer). */
export const AURA_DATA_CHANGED = 'aura-data-changed';

export type AuraDataChangedDetail = {
  /** Логический тип изменения данных (см. `prefixesForAuraDataType`). */
  type: string;
};
...
/** Префиксы ключей кэша (`StatsCache.generateKey` начинается с `mode_`). */
