...
export function EvilGlowingStackedBarChart({ data, series, chartConfig, vertical = false, meta, mode }: Props) {
  return (
    <ChartContainer
      config={chartConfig}
    ...
    >
      <BarChart accessibilityLayer data={data} layout={vertical ? 'vertical' : 'horizontal'} margin={vertical ? { left: -15 } : undefined}>
        ...
        {[...series].reverse().map((item) => (
          <Bar key={item.key} stackId="a" barSize={10} dataKey={item.key} fill={item.color} radius={0} isAnimationActive={false} />
        ))}
      </BarChart>
    </ChartContainer>
  );
